...
((exports) => {
  ...
  const addAnswerOptionButtonSelector = ".add-answer-option";
  ...
  const createDynamicFieldsForAnswerOptions = (fieldId) => {
    ...
    return createDynamicFields({
      ...
      addFieldButtonSelector: addAnswerOptionButtonSelector,
  ...
  const hideDeletedQuestion = ($target) => {
    const inputDeleted = $target.find(deletedInputSelector).val();
